...
select 
    ...
    속성 | 식 등 -- 순서가 섞이면 안됨
	[from 테이블명]
    [where 조건절] -- 조건에 맞는 결과를 조회; == 가 아닌 =을 이용하여 비교, 같지 않은 경우는 !=을 이용
...
select * from member 
	order by me_id
    limit 0, 2;
-- 회원들을 아이디 순으로 오름차순으로 정렬하여 2번째 2명만 조회
select * from member
	order by me_id
    limit 2,2;
-- 회원들을 아이디 순으로 오름차순으로 정렬하여 3번째 2명만 조회
select * from member
	order by me_id
    limit 4,2;
